...
export default function Bar({ queue }) {
	const data = [];
	queue.forEach((entry) => {
		const onlyTime = entry.loggedAt.substring(entry.loggedAt.indexOf(" ") + 1);
		data.push({ inQueue: entry.inQueue, date: onlyTime });
	});
	...
	return (
		<div className="bar">
			...
			<Chart
				...
				scale={{ inQueue: { min: 0, max: 25 } }}
			>
				<Tooltip shared />
				<LineAdvance shape="smooth" point area position="date*inQueue" color="rebeccapurple" />
			</Chart>
		</div>
	);
}
